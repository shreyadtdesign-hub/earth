...
import { useScrollScrubHero } from "@/hooks/useScrollScrubHero";
import { useReducedMotion } from "@/hooks/useReducedMotion";
import { heroContent, heroPoster, heroVideos } from "@/data/hero";
import { Button } from "@/components/ui/Button";
import { VideoTransition } from "./VideoTransition";

/** Scroll distance the hero stays pinned for while video1/video2 scrub through. */
const HERO_SCRUB_HEIGHT = "300vh";

export function VideoHero() {
  const reducedMotion = useReducedMotion();
  const { wrapperRef, videoARef, videoBRef, textRef } = useScrollScrubHero(reducedMotion);

  return (
    <section ref={wrapperRef} className="relative" style={{ height: reducedMotion ? "100dvh" : HERO_SCRUB_HEIGHT }}>
      ...
      <div className="sticky top-0 flex h-[100dvh] items-center justify-center overflow-hidden bg-charcoal">
        <div className="absolute inset-0">
          {reducedMotion ? (
            <Image src={heroPoster.src} alt={heroPoster.alt} fill priority sizes="100vw" className="object-cover" />
          ) : (
            <>
        ...
        <div ref={textRef} className="relative z-10 flex origin-center flex-col items-center px-6 text-center will-change-transform">
